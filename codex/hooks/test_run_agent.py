...
class TestGetSessions(unittest.TestCase):
    ...
    def test_missing_sessions(self):
        self.assertEqual(
            run_agent.get_sessions({"artifact": {"codex": {}}}), []
        )
...
class TestResolveSession(unittest.TestCase):
    ...
    def test_miss_returns_none(self):
        # Codex cannot pre-assign a session ID; new sessions report None
        # and is_resume=False so the caller captures the ID from the stream.
        sessions = [
            {"type": "executor", "name": "", "id": "abc-123"},
        ]
        session_id, is_resume = run_agent.resolve_session(
...
class TestUpdateSessions(unittest.TestCase):
    ...
    def test_none_session_id_leaves_sessions_untouched(self):
        # When Codex fails to emit a session_id, we should not store a bogus entry.
        sessions = [{"type": "executor", "name": "", "id": "exec-id"}]
        result = run_agent.update_sessions(sessions, "verifier", "security", None)
...
class TestBuildPayloadPatch(unittest.TestCase):
    ...
    def test_multiple_sessions(self):
        ...
        patch = run_agent.build_payload_patch(sessions)
        self.assertEqual(
            patch["payload_patch"]["artifact"]["codex"]["sessions"], sessions
        )
...
class TestWritePayloadPatch(unittest.TestCase):
    def test_writes_valid_yaml(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                data["payload_patch"]["artifact"]["codex"]["sessions"],
                sessions,
            )
...
class TestMergeSessionsIntoPatch(unittest.TestCase):
    def test_empty_existing(self):
        ...
        self.assertEqual(
            result["payload_patch"]["artifact"]["codex"]["sessions"], sessions
        )
...
class TestReadPayloadFromFile(unittest.TestCase):
    def test_reads_json_file(self):
        ...
        try:
            ...
            self.assertEqual(
                payload["artifact"]["codex"]["sessions"][0]["id"], "file-id"
            )
        finally:
            os.unlink(path)
...
class TestReadPayloadFromString(unittest.TestCase):
    def test_valid_json(self):
        data = json.dumps({"artifact": {"codex": {"sessions": []}}})
        payload = run_agent.read_payload_from_string(data)
        self.assertEqual(payload["artifact"]["codex"]["sessions"], [])
...
class TestExtractSessionId(unittest.TestCase):
    def test_thread_id(self):
        ...
        self.assertEqual(run_agent.extract_session_id(event), "abc-123")

    def test_session_id(self):
        event = {"session_id": "sess-1"}
        self.assertEqual(run_agent.extract_session_id(event), "sess-1")

    def test_conversation_id(self):
        event = {"conversation_id": "conv-1"}
        self.assertEqual(run_agent.extract_session_id(event), "conv-1")

    def test_none_when_missing(self):
        self.assertIsNone(run_agent.extract_session_id({"type": "turn.started"}))

    def test_none_on_non_dict(self):
        self.assertIsNone(run_agent.extract_session_id(None))
        self.assertIsNone(run_agent.extract_session_id("string"))
        self.assertIsNone(run_agent.extract_session_id(42))

    def test_thread_id_preferred(self):
        event = {"thread_id": "t-1", "session_id": "s-1", "conversation_id": "c-1"}
        self.assertEqual(run_agent.extract_session_id(event), "t-1")
